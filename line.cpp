...
void drawLine(Canvas& canvas, Vec2 a, Vec2 b, double width, unsigned char color) {
    ...
    double minY = std::min(a.y, b.y) - halfWidth - 1;
    double maxY = std::max(a.y, b.y) + halfWidth + 1;
    ...
    endX = std::min(canvas.width - 1, endX);
    startY = std::max(0, startY);
    endY = std::min(canvas.height - 1, endY);
        ...
        for (int px = startX; px <= endX; ++px) {
            ...
            for (int sy = 0; sy < S; ++sy) {
                for (int sx = 0; sx < S; ++sx) {
                    ...
                    double dist = std::min(distSeg, std::min(distA, distB));
...
int main() {
    ...
    std::cout << "生成完成：line.ppm" << std::endl;
